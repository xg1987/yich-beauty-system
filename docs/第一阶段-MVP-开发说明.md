...
已经完成本地 SQLite、HTTP API、登录会话、前端 API 数据源切换，以及 Cloudflare Pages Functions + D1 线上后端。下一步需要继续强化权限中间件、拆分页面模块，并开始真实门店业务增强。

当前已补齐第一版全额退款、部分退款、提成冲销、协作员工提成拆分、会员退卡、会员卡流水、操作日志、服务端权限拦截、员工数据范围过滤、项目次数卡、预约冲突检测、员工不可预约时段和财务日结。后续还需要把权限从“接口级/角色级”继续细化到字段级。

### 2. 页面拆分

当前为了快速推进，页面集中在 `src/App.tsx`。进入第二阶段业务开发前建议拆分：
